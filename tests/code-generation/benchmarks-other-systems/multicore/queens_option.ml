...
type 'a computation =
  ...
  | Call :
      ('eff_arg, 'eff_res) effect_type * 'eff_arg * ('eff_res -> 'a computation)
      -> 'a computation
...
type 'a effect_clauses =
  ...
  | Cons :
      ('eff_arg, 'eff_res) effect_type
      * ('eff_arg -> ('eff_res -> 'a computation) -> 'a computation)
      * 'a effect_clauses
      -> 'a effect_clauses
...
let rec find_case :
          'eff_arg 'eff_res. ('eff_arg, 'eff_res) effect_type ->
          'a effect_clauses -> 'eff_arg -> ('eff_res -> 'a computation) ->
          'a computation =
 fun eff eff_clauses ->
  match eff_clauses with
  | Nil -> fun x k -> Call (eff, x, k)
...
let call (eff : ('a, 'b) effect_type) (arg : 'a) (cont : 'b -> 'c computation) :
    'c computation =
  Call (eff, arg, cont)

let effect_type eff arg = call eff arg value
