...
type (_, _) effect += Effect_Fail : (unit, int) effect

let rec _loop_latent_5 _n_6 =
  if _n_6 = 0 then call Effect_Fail () (fun _result_2 -> value _result_2)
  else if _n_6 < 0 then call Effect_Fail () (fun _result_4 -> value _result_4)
  ...
  handler
    {
      ...
      effect_clauses =
        (fun (type a b) (x : (a, b) effect) : (a -> (b -> _) -> _) ->
          match x with
          | Effect_Fail -> fun (() : unit) (_k_14 : int -> _) -> _k_14 0
...
let _test_once_18 () =
  let rec _loop_latent_9 _n_6 =
    if _n_6 = 0 then 0 else if _n_6 < 0 then 0 else _loop_latent_9 (_n_6 - 1)
  in
